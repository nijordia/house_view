...
// SVG Path Data for each district in Barcelona
export const DISTRICT_PATHS: DistrictPath[] = [
{
        label: "Ciutat Vella",
...
// Mapping from display names to data identifiers
export const SVG_TO_DATA_COMMUNE: Record<string, string> = {
...
// Utility function to get all district names
export const getDistrictNames = (): string[] => {
    return DISTRICT_PATHS.map(district => district.label);
};

// Utility function to validate district data consistency
export const validateMapData = (): boolean => {
    const pathLabels = new Set(DISTRICT_PATHS.map(d => d.label));
    const labelLabels = new Set(DISTRICT_LABELS.map(d => d.label));
    const communeKeys = new Set(Object.keys(SVG_TO_DATA_COMMUNE));
    
    return pathLabels.size === labelLabels.size && 
           pathLabels.size === communeKeys.size &&
           [...pathLabels].every(label => labelLabels.has(label) && communeKeys.has(label));
};